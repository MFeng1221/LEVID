...
class MaxPressureGreenAgent(Agent):

    def __init__(self, dic_agent_conf, dic_traffic_env_conf, dic_path, cnt_round, intersection_id):

        super(MaxPressureGreenAgent, self).__init__(dic_agent_conf, dic_traffic_env_conf, dic_path, intersection_id)
    ...
    def choose_action(self, count, state):
        ...
        if np.sum(state["lane_num_sp_vehicle"]) > 0:
            print("lane_num_sp_vehicle:",state["lane_num_sp_vehicle"])
            #print()
            phase_sp_vehicle_num = [0] * 4
            phase_sp_vehicle_num [0] = state["lane_num_sp_vehicle"][1] + state["lane_num_sp_vehicle"][4]
            phase_sp_vehicle_num [1] = state["lane_num_sp_vehicle"][7] + state["lane_num_sp_vehicle"][10]
            phase_sp_vehicle_num [2] = state["lane_num_sp_vehicle"][0] + state["lane_num_sp_vehicle"][3]
            phase_sp_vehicle_num [3] = state["lane_num_sp_vehicle"][6] + state["lane_num_sp_vehicle"][9]
            cur_phase = phase_sp_vehicle_num.index(np.max(phase_sp_vehicle_num))
            print("action:",cur_phase)
            return cur_phase
